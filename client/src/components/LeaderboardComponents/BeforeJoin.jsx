  ...
  return (
    <div>
      ...
      <div onClick={(e_) => e_.stopPropagation()} className={`absolute max-w-[450px] rounded-lg bg-gradient-to-b from-[#bacaff] to-[#ffffff] p-6 drop-shadow-lg  dark:text-black ${openModal ? 'opacity-1 duration-300' : 'scale-110 opacity-0 duration-150'}`}>
        ...
        <div className=" text-[1.3rem] font-semibold text-center mt-5 ">
            <h3>Do you want to join to <br /> earn through leaderboard?</h3>
        </div>
        <div className=" flex items-center justify-center mt-6 gap-5 ">
            <button onClick={handleJoinButton} className=" text-[1.2rem] font-semibold text-[#fff] bg-blue-500 px-10 py-1 rounded-md ">Yes</button>
            <button onClick={()=>setOpenModal(false)} className=" text-[1.2rem] font-semibold text-[#fff] bg-blue-500 px-10 py-1 rounded-md ">No</button>
        </div>
        </div>
        </div>
        </div>
